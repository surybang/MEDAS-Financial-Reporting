...
Si tout va bien en accédant à l'URL renseigné dans l'Ingress, votre application devrait être accessible via votre navigateur préféré. Par contre  tout n'est pas totalement fonctionnel. (encore une fois)

## Alimenter le stockage : lancer le pipeline

Bon... il manque un truc là. On a déployé l'application, elle est accessible, mais elle affiche une erreur : impossible de charger les données. Et c'est logique : notre `MinIO` est tout neuf et vide. On le lance quand, notre pipeline ? Même si tout est statique, il faut bien l'exécuter au moins une fois pour produire les données nettoyées et le reporting que l'application va lire.

C'est le chaînon manquant. Jusqu'ici on a déployé la brique qui **lit** (`Streamlit`), mais rien n'a encore **écrit** dans le bucket.

::: {.callout-note}
## Deux data products, un point de rencontre
Notre projet contient en réalité deux *data products* de natures différentes. Le **package** produit et persiste : il lit les données source, les transforme et sauvegarde le résultat sur `MinIO`.**Streamlit** lit et expose : il se contente d'afficher ce que le package a généré. Le `MinIO` est le point de rencontre entre les deux, l'endroit où le premier dépose ce que le second consomme. C'est pour ça que la persistance compte : sans elle, on perd la trace de la transformation et l'application n'a plus rien à montrer.
:::

Pour exécuter le pipeline, on pourrait lancer la commande à la main dans un terminal mais c'est fastidieux, fragile et peu intuitif. Il faudrait réutiliser le `VS Code` d'admin et taper des commandes dans le `Terminal`. On va plutôt le faire proprement avec un **Job Kubernetes** : une tâche ponctuelle qui réutilise notre image mais lancée avec la commande `reporting` (`uv run reporting`). Le `Job` crée le bucket si besoin puis exécute le pipeline.
...
- Commitez et poussez : `ArgoCD` déploiera le Job, qui créera le bucket et remplira le stockage
:::
...
D'abord, notre Job tourne une fois. En production, un reporting se régénère souvent à intervalle régulier (chaque nuit, chaque semaine, chaque mois ...) pour intégrer de nouvelles données. On utiliserait alors un déclenchement planifié plutôt qu'une exécution unique.

Ensuite, regrouper le pipeline d'alimentation et l'application dans le même dépôt de déploiement est un raccourci de démonstration. Dans une vraie architecture, on **séparerait** ces deux responsabilités : l'alimentation du bucket (ingestion, transformation) d'un côté, l'exposition via `Streamlit` de l'autre. Deux mondes, deux cycles de vie.
...
Et voilà. Cette fois, l'application est pleinement fonctionnelle : les données se chargent, les graphiques s'affichent et le reporting est téléchargeable. Vous venez de boucler la chaîne complète, du code poussé jusqu'à un data product en ligne, accessible à n'importe qui via une simple URL.
