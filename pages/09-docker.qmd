...
Il faut quand même nuancer un peu ces propos concernant `Docker`, ce dernier ne permet pas résoudre tous les problèmes d'environnement, il contribue très fortemment à les limiter sans les éliminer. L'architecture de la machine (ARM vs x86), tout ce qui vit *hors* du conteneur (réseau, volumes, secrets, services voisins) ou un build mal figé peuvent encore vous jouer des tours. Finalement, la bonne promesse de `Docker` n'est pas "ça marchera partout" mais "le même environnement applicatif partout". Sans vouloir vous spoiler, vous allez en faire l'expérience très vite : notre image va build et démarrer correctement, sans que l'application soit pour autant pleinement fonctionnelle. 👀
