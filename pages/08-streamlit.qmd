...
`Streamlit` est un framework `Python` qui permet de construire des applications web de visualisation de données sans écrire une ligne de `HTML`, `CSS` ou `JavaScript` (bien qu'on pourrait en injecter 👀). On écrit du `Python`, `Streamlit` se charge de l'interface.
