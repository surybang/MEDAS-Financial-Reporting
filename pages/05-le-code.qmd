...
La première étape consiste à remplir `config.py` à la racine de `src/medas_financial_reporting/`. Ce module centralise toute la configuration commune aux deux sous-packages : accès MinIO, chemins distants, constantes Excel et indicateurs.

Quelques contraintes à respecter :

- **Pas d'artefacts locaux persistants,** le template et le reporting final vivent dans `MinIO`
- **Un dossier temporaire** `tmp/` est créé au runtime pour écrire le fichier avant upload
...
Vous remarquez que `config.py` a évolué par rapport à la [version précédente](https://github.com/surybang/MEDAS-TP-Financial-Reporting/blob/refactor/src/medas_tp_financial_reporting/config.py). Dans le projet initial, ce fichier était minimal : une URL, les indicateurs, quelques chemins locaux. C'était suffisant pour un script autonome.
...
Plus précisement, quatre choses ont changé :
...
Tout comme `config.py`, ce module vit à la racine de `src/medas_financial_reporting/` car il est partagé entre les deux sous-packages. En effet, la génération du reporting interagit avec `MinIO` tout comme `Streamlit` en aura aussi besoin pour exposer le fichier final à l'utilisateur.
...
def init_minio_structure(fs: s3fs.S3FileSystem, bucket: str) -> None:
    """Génère le bucket et la structure de dossiers si elle n'existe pas."""
    ...
    logger.info("Structure MinIO prête")
